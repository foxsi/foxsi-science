function inverse_exponential, x, a
; EXPONENTIAL.PRO
...
function inverse_exponential_fit, x, y, guess = guess, $
                          sigma = sigma, errors = errors
;+
; NAME:
;   EXPONENTIAL_FIT
; PURPOSE:
;   Fit an exponential to the data of the form y=a*exp(-b/x)+c.  It's
;   just a wrapper for LMFIT, as always.
;
; CALLING SEQUENCE:
;   fit=inverse_exponential_fit(x, y[, errors=errors, guess=guess,
;   sigma=sigma])
;
; INPUTS:
...
;-
  ...
  if not keyword_set(guess) then begin
    ...
    guess = [exp(coeffs[0]), coeffs[1], 0.1]
  endif
  a = guess
  for i = 0, 300 do begin
    fit = lmfit(x, y, a, $
                measure_errors = errors, /double, sigma = sigma, $
                function_name = 'inverse_exponential', conv = conv)
