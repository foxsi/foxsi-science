...
if det eq 0 then img0=foxsi_image_solar(data_lvl2_d0,0,psize=pix,erange=erange,trange=tr,thr_n=4., size=[imdim,imdim])
if det eq 1 then img1=foxsi_image_solar(data_lvl2_d1,1,psize=pix,erange=erange,trange=tr,thr_n=4., size=[imdim,imdim])
if det eq 2 then img2=foxsi_image_solar(data_lvl2_d2,2,psize=pix,erange=erange,trange=tr,thr_n=4., size=[imdim,imdim])
if det eq 3 then img3=foxsi_image_solar(data_lvl2_d3,3,psize=pix,erange=erange,trange=tr,thr_n=4., size=[imdim,imdim])
if det eq 4 then img4=foxsi_image_solar(data_lvl2_d4,4,psize=pix,erange=erange,trange=tr,thr_n=4., size=[imdim,imdim])
if det eq 5 then img5=foxsi_image_solar(data_lvl2_d5,5,psize=pix,erange=erange,trange=tr,thr_n=4., size=[imdim,imdim])
if det eq 6 then img6=foxsi_image_solar(data_lvl2_d6,6,psize=pix,erange=erange,trange=tr,thr_n=4., size=[imdim,imdim])
 
if det eq 0 then map0 = make_map( img0, xcen=0., ycen=0., dx=pix, dy=pix )
if det eq 1 then map1 = make_map( img1, xcen=0., ycen=0., dx=pix, dy=pix )
if det eq 2 then map2 = make_map( img2, xcen=0., ycen=0., dx=pix, dy=pix )
if det eq 3 then map3 = make_map( img3, xcen=0., ycen=0., dx=pix, dy=pix )
if det eq 4 then map4 = make_map( img4, xcen=0., ycen=0., dx=pix, dy=pix )
if det eq 5 then map5 = make_map( img5, xcen=0., ycen=0., dx=pix, dy=pix )
if det eq 6 then map6 = make_map( img6, xcen=0., ycen=0., dx=pix, dy=pix )
 ;plot_map, map6, /limb, cen=flare, fov=2, /cbar
 
 ; Selecting subregions...'
 
if det eq 0 then raw0 = shift_map( make_submap( map0, cen=flare, fov=fov), -flare[0], -flare[1] )
if det eq 1 then raw1 = shift_map( make_submap( map1, cen=flare, fov=fov), -flare[0], -flare[1] )
if det eq 2 then raw2 = shift_map( make_submap( map2, cen=flare, fov=fov), -flare[0], -flare[1] )
if det eq 3 then raw3 = shift_map( make_submap( map3, cen=flare, fov=fov), -flare[0], -flare[1] )
if det eq 4 then raw4 = shift_map( make_submap( map4, cen=flare, fov=fov), -flare[0], -flare[1] )
if det eq 5 then raw5 = shift_map( make_submap( map5, cen=flare, fov=fov), -flare[0], -flare[1] )
if det eq 6 then raw6 = shift_map( make_submap( map6, cen=flare, fov=fov), -flare[0], -flare[1] )
 
if det eq 0 then raw0 = make_map( raw0.data[0:dim-1,0:dim-1], dx=pix, dy=pix )
if det eq 1 then raw1 = make_map( raw1.data[0:dim-1,0:dim-1], dx=pix, dy=pix )
if det eq 2 then raw2 = make_map( raw2.data[0:dim-1,0:dim-1], dx=pix, dy=pix )
if det eq 3 then raw3 = make_map( raw3.data[0:dim-1,0:dim-1], dx=pix, dy=pix )
if det eq 4 then raw4 = make_map( raw4.data[0:dim-1,0:dim-1], dx=pix, dy=pix )
if det eq 5 then raw5 = make_map( raw5.data[0:dim-1,0:dim-1], dx=pix, dy=pix )
if det eq 6 then raw6 = make_map( raw6.data[0:dim-1,0:dim-1], dx=pix, dy=pix )
 ...
 if det eq 0 then raw=raw0
 if det eq 1 then raw=raw1
 if det eq 2 then raw=raw2
 if det eq 3 then raw=raw3
 if det eq 4 then raw=raw4
 if det eq 5 then raw=raw5
 if det eq 6 then raw=raw6
 ...
 !p.multi=0
 plot_map, aia[0]
 plot_map, shift_map(deconv_map[6], flare_real[0], flare_real[1]), /over, lev=[5,10,20,40,60,80], /per